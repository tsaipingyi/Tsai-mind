...
const fmtVal = (v) => (typeof v === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(v) ? md(v) : v == null ? '无' : String(v));
const line = (i) => `- ${i.title}（${i.project}${i.path ? ' / ' + i.path : ''}）· ${i.owner} · ${md(i.due)}` + (i.days > 0 ? ` · 逾期 ${i.days} 天` : '');
const sections = [];
...
if (pending.length) sections.push(`【待确认 ${pending.length}】\n${pending.map((p) => `- ${p.title}（${p.project}）：${fieldName[p.field] ?? p.field} ${fmtVal(p.from)} → ${fmtVal(p.to)}${p.reason ? '，理由：' + p.reason : ''}`).join('\n')}`);
...
const pendingHtml = pending.length ? `<h3 style="margin:18px 0 6px;font-size:15px">待确认 <span style="color:#6B6B6B;font-weight:400">${pending.length}</span></h3>` + pending.map((p) => `<div style="background:#FFF1E8;border-left:3px solid #F26B1D;padding:10px 12px;margin:0 0 8px;border-radius:0 8px 8px 0"><b>${esc(p.title)}</b> <span style="color:#6B6B6B">${esc(p.project)}</span><div style="font-family:Menlo,monospace;font-size:13px;margin-top:4px">${esc(fieldName[p.field] ?? p.field)} ${esc(fmtVal(p.from))} → ${esc(fmtVal(p.to))}</div>${p.reason ? `<div style="color:#6B6B6B;font-size:13px">${esc(p.reason)}</div>` : ''}</div>`).join('') : '';
