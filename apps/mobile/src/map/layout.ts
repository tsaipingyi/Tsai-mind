// Same layout rules as apps/web/src/editor/layout.ts (kept in sync by hand; the web package is not importable here).
...
/** Rough text width for truncation inside SVG boxes (CJK ≈ 1em, latin ≈ 0.55em). */
...
export function truncate(s: string, fontSize: number, maxW: number): string {
  if (textWidth(s, fontSize) <= maxW) return s;
  let out = '';
  for (const ch of s) {
    if (textWidth(out + ch + '…', fontSize) > maxW) break;
    out += ch;
  }
  return out + '…';
}
